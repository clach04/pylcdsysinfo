...
class Simulation:
	...
	def run(self):
		...
		lcd = LCDSysInfo()
		lcd.clear_lines(TextLines.ALL, bg)
		lcd.dim_when_idle(False)
		lcd.set_brightness(255)
		lcd.save_brightness(255, 255)
		...
		while 1:
			...
			lcd.fill_rectangle_rel(75, 105, 110, 110, black )
			...
			for v in self.vertices:
				...
				p = r.project(320, 240, 400, 16)
				# Put the point in the list of transformed vertices
				t.append(p)

			'''
			if(zzyzx == 1):
				lcd.fill_rectangle(10, 20, 160, 120, cc16( 0, 0, 0 ) )
			else:
				zzyzx = 1
			'''
			...
			for f in self.faces:
				...
				lcd.draw_line( int(t[f[0]].x), int(t[f[0]].y), int(t[f[1]].x), int(t[f[1]].y), white )
				lcd.draw_line( int(t[f[1]].x), int(t[f[1]].y), int(t[f[2]].x), int(t[f[2]].y), white )
				lcd.draw_line( int(t[f[2]].x), int(t[f[2]].y), int(t[f[3]].x), int(t[f[3]].y), white )
				lcd.draw_line( int(t[f[3]].x), int(t[f[3]].y), int(t[f[0]].x), int(t[f[0]].y), white )
